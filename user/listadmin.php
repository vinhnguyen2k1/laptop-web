<?php
    ...
    if (!empty($_GET['limit']) && !empty($_GET['page'])) {
        ...
        $rowUser = mysqli_query($con, "SELECT * FROM `user` WHERE `status` LIKE 'admin'")->num_rows;
        $totalpages = ceil($rowUser / $getLimit);
        if(!empty($_GET['page']) && $_GET['page'] < 0){
            $user = mysqli_query($con, "SELECT * FROM `user` WHERE `status` LIKE 'admin' LIMIT ".$getLimit." OFFSET 1");
        }else
        $user = mysqli_query($con, "SELECT * FROM `user` WHERE `status` LIKE 'admin' LIMIT ".$getLimit." OFFSET ".$offset."");
// xóa user
