<?php
    ...
    if (!empty($_GET['limit']) && !empty($_GET['page'])) {
        ...
        $rowUser = mysqli_query($con, "SELECT * FROM `user` WHERE `status` LIKE 'user'")->num_rows;
        $totalpages = ceil($rowUser / $getLimit);
        if(!empty($_GET['page']) && $_GET['page'] < 0){
            $user = mysqli_query($con, "SELECT * FROM `user` WHERE `status` LIKE 'user' LIMIT ".$getLimit." OFFSET 1");
        }else
        $user = mysqli_query($con, "SELECT * FROM `user` WHERE `status` LIKE 'user' LIMIT ".$getLimit." OFFSET ".$offset."");
// xóa user
...
<div class="box-list">
    ...
    <div class="box-header box-header-style">
        ...
        <div class="box-header__del style-boder"><a href="./admin.php?listuser&delete_user&id=<?=$value['idUser']?>"
